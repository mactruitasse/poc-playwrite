...
async def create_session():
    ...
    if MCP_AUTO_INIT:
        ...
        log.warning("MCP_AUTO_INIT=false: skipping MCP initialize during /sessions")
...
async def proxy_any(session_id: str, path: str, request: Request):
    ...
    for h in ("connection", "Connection", "content-length", "Content-Length"):
        headers.pop(h, None)
    ...
    # MCP requires Accept to include both application/json and text/event-stream,
    # and requires mcp-session-id after initialize.
    ...
    def _strip_hop_by_hop(resp_headers: "httpx.Headers") -> dict:
        hop = {
            "connection",
            "keep-alive",
            "proxy-authenticate",
    ...
    def _client_initialize_ok(request_body: bytes) -> Response:
        rid = None
        try:
            j = json.loads((request_body or b"{}").decode("utf-8", errors="replace"))
            rid = j.get("id", None)
        except Exception:
            rid = None
    ...
    async def _reinit_mcp_and_retry(client_http: httpx.AsyncClient, stream: bool):
        ...
        return await client_http.send(req, stream=stream)
                    ...
                    try:
                        ...
                        async for chunk in r.aiter_bytes():
                            if first_chunk:
                                first_chunk = False
                                try:
                                    txt_chunk = chunk.decode("utf-8", errors="replace")
                                    lines = txt_chunk.splitlines(True)
                                    out_lines = []
                                    for line in lines:
                                        ln = line.rstrip("\r\n")
                                        if ln.startswith("data: /"):
                                            rel = ln[len("data: "):].strip()
                                            abs_url = urljoin(base_url, rel.lstrip("/"))
                                            out_lines.append("data: " + abs_url + "\n")
                                            continue
                                        out_lines.append(line)
                                    chunk = "".join(out_lines).encode("utf-8")
                                except Exception:
                                    pass
                            yield chunk
                    except httpx.RemoteProtocolError as e:
                        log.warning("Upstream SSE closed abruptly: %s", e)
                        return
                    finally:
                        try:
                            await r.aclose()
